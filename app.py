...
st.header("Commercial Activity by Town")

activity_for_towns = st.selectbox(
    "Select an activity:",
...
if len(selected_districts) == 1:
    ...
    town_data = town_activity_df[
        town_activity_df['District'] == selected_district
    ][['Town', activity_for_towns]]

    town_data = town_data[
        town_data[activity_for_towns] > 0
    ].sort_values(activity_for_towns)

    fig_town = px.bar(
        town_data,
        x=activity_for_towns,
        y='Town',
        orientation='h',
        title=f'{activity_for_towns} by Town in {selected_district}',
        labels={
            activity_for_towns: 'Number of Activities',
            'Town': 'Town'
        }
    )
    ...
    st.info("Select one district to view its towns.")
